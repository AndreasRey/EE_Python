...
extent = geemap.geojson_to_ee('./data/input/NER/NER_Extent_Test.geojson')
aoi = geemap.geojson_to_ee('./data/input/NER/NER_Intersection_Test.geojson')
nameProp = 'uid'
subNameField = 'rowcacode1'
trainingDataset = geemap.geojson_to_ee('./data/input/NER/NER_Training_Test.geojson')
...
table = geemap.geojson_to_ee('./data/input/NER/NER_Intersection_Test.geojson')
count = 0
values = []
...
with open('./data/input/NER/NER_Intersection_Test.geojson', 'r') as file:
    data = json.load(file)
    ...
    for x in data['features']:
        ...
        # geemap.ee_export_image(singleImage, './data/output/NER_images/' + str(uid) + '.tif', 30)
        ...
        if subCroplands_Classification.size().getInfo() > 0:
          # geemap.ee_export_geojson(subCroplands_Classification, './data/output/NER_vectors/' + str(uid) + '.geojson')
        ...
        with open('./data/output/NER_values/' + str(uid) + '.json', 'w') as single_valueFile:
          json.dump(props, single_valueFile)
          single_valueFile.close()

    # all_valuesFile = open('./data/output/GMB_values/' + 'VALUES' + '.json', 'w')
    with open('./data/output/NER_values/' + 'VALUES' + '.json', 'w') as all_valuesFile:
      json.dump(values, all_valuesFile)
      all_valuesFile.close()
    
    fromJsonToCsv('./data/output/NER_values/' + 'VALUES' + '.json', './data/output/NER_values/' + 'VALUES' + '.csv')
